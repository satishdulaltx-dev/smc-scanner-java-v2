...
public class SpyMomentumResearchService {
    ...
    public Map<String,Object> run(LocalDate start,LocalDate end){
        ...
        TreeMap<LocalDate,List<OHLCV>> sessions=completeSessions(bars);List<LocalDate> dates=new ArrayList<>(sessions.keySet());
        List<BacktestService.TradeResult> trades=new ArrayList<>();List<Map<String,Object>> audit=new ArrayList<>();int skipped=0;
        ...
        out.put("coverage",Map.of("complete_sessions",dates.size(),"skipped_test_sessions",skipped));
        out.put("warnings",List.of("Underlying SPY replay only; historical option quotes are unavailable"));
    ...
    private static TreeMap<LocalDate,List<OHLCV>> completeSessions(List<OHLCV> bars){TreeMap<LocalDate,List<OHLCV>> out=new TreeMap<>();for(OHLCV b:bars){var z=Instant.ofEpochMilli(b.getTimestamp()).atZone(ET);LocalTime t=z.toLocalTime();if(!t.isBefore(LocalTime.of(9,30))&&t.isBefore(LocalTime.of(16,0)))out.computeIfAbsent(z.toLocalDate(),k->new ArrayList<>()).add(b);}out.values().forEach(v->v.sort(Comparator.comparingLong(OHLCV::getTimestamp)));out.entrySet().removeIf(e->!complete(e.getKey(),e.getValue()));return out;}
